  ...
  return (
    <div className='w-full grid grid-cols-2 h-full text-white/70'>
      {skills.map((item: dataSkill, index: number) => (
        <div className={`flex items-center justify-center border border-white/10 hover:bg-[#${item.color}] duration-300  hover:border-white/20  hover:text-white hover:shadow-2xl hover:shadow-[#${item.color}] ${hoverSkill === index ? 'delay-700 duration-300' : ''}`}
